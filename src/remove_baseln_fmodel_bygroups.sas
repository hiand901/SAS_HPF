     ...
     ** to remove them from the input data.                                              **;
         ...
         %if &baseline_forced_obs > 0 %then %do ;
             ...
             ** remainder of process is the 'overwrite_best_model' table.                   **;
             data baseline_forced_models_denorm ;
                 ...
                 %do i = 1 %to &num_cat_vars ;
                     if upcase(byvar) = upcase("&&cat_var_&i") then &&cat_var_&i = byvarvalue ;
                 %end;
     ...
     %else %if &d_or_f = F %then %do ;
         ...
         data baseline_forced_models_denorm ;
             ...
             %do i = 1 %to &num_cat_vars ;
                 %if &i > 1 %then else ;
                 if upcase(compress(byvar)) = upcase(compress("&&cat_var_&i"))
                 then &&cat_var_&i = byvarvalue ;
             %end;
     ...
     %if &baseline_forced_obs > 0 %then %do ;
         ...
         ** Removed by variables from the by statement, and added a nodupkey  **;
         ** to allow for the change (previously made) to the following merge, **;
         ** which was changed by someone to likewise remove the by variables  **;
         ** from the merge by statement.                       - AH 08162011  **;
         proc sort data = baseline_forced_models_denorm nodupkey;
         ...
         ** by_variables that are associated with the Baseline forced models.             **;
         ...
         data &input_demand_table
             ...
             if dmnd ;
             /* These statements commented out 08162011 *
             %if &num_cat_vars > 0 %then
             ...
             if first.prop_code ;
               then do ; */
                 ...
                 if base then do ;
                    by_groups_removed + 1;
                    * Following statement added 08162011 *;
                    if first.prop_code then do;
                        output prop_codes_all_bgs_removed ;
                        output removed_by_groups ;
                    end;
                 end;
             /* end; */
             /* Commented-out 08162011 *
