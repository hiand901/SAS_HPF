...
%macro run_forecast (top_dir, 
    ...
    ** Read the config file settings **;
    ...
    data config ;
        ...
        if _n_ = 1 then do ;
            ...
            if tz_pos > 0 then do;
                tmz_dir = substr(top_dir, tz_pos, 3); 
                call symput('tzr', substr(left(tmz_dir),3,1));
            end;
        end;
    ...
    ** Group or Transient.                                                          **;
    ...
    %else %if %upcase(&group_or_transient) = T %then %do ;
        %if &rates_or_rooms = rooms %then %let demand_col = total_arvl ;
        %else %let demand_col = rate ;
    ...
    %if %upcase(&prop_code) = ALL %then %do ;
        ...
        ** Obtain the input prop_code list ** ;
        %if &run_mode ne D and &prop_code_input_list_name ne %str() %then %do;
            ...
            ** Added 01132012. Call to build_prop_list to read the input prop codes and **;
            ** cap1 and cap2 values from the [/oy/fcst/data/tzN/in] location.           **;

            %build_prop_list(&prop_code_input_list_path,
                             &prop_code_input_list_name,
                             &tzr,
                             work.prop_code_list,
                             &top_dir,
