...
%macro run_diagnose (config_file_path,config_file_name);
*********************************************************************************************;
...
%include "/fcst/HPF/SAS/code/Baseline_fcst.sas";
...
data config ;
		...
		/****
		if trim(left(key))  = 'prop_limit' then do;
     		max_property    = input( trim( left(value) ), 4.0); 			
            put " MAX_PROPERTY  " max_property;  
            call symput ( 'max_property', max_property);
        end;
         ****/
...
/** BUILD transient_prop_list                                                     ;
** Added by VASAN on Jan 4, 2010. prop_list file is constructed from the forecast ;
  ...
  %if &dataobs > 0 %then %do ;
    ...
    ** room config changes    Feb-2010                                       ;    
    ...
    if cap2 = 0 and prop_rmc_total > 0;
	 ...
	 merge demand_data_summarized1 (in =inA) room_config_list(in=inB);
	 by prop_code;
     if (inA) and not (inB); 
     run; 
...
%run_forecast(
    ...
    run_mode= &run_mode   ,diagnose_date=&diag_as_of_date,
    run_date= &run_date_1 ,
    overide_out_table= hpfsys.results1 );
data hpfsys.results1;
set hpfsys_test;
run;
...
%run_forecast(
    ...
    config_file_name = &config_file_name ,
    run_mode= &run_mode   ,
    run_date= &run_date_2 , diagnose_date=&diag_as_of_date,
    overide_out_table= hpfsys.results2 );

data hpfsys.results2;
set hpfsys_test;
run;
/**********************************************************************;
*Run with D ;
...
%run_forecast(
  ...
  config_file_path = &config_file_path ,
    config_file_name = &config_file_name ,
    run_mode= &run_mode   ,
    run_date= &run_date_3 , diagnose_date=&diag_as_of_date,
    overide_out_table= hpfsys.results3 );

data hpfsys.results3;
set hpfsys_test;
run;
...
/** combine HPF fcst with Baseline Fcst ; */
   ...
   data comb_fcst;
   merge hpfsys.hpf_forecast (in=inA) hpfsys.baseline_forecast (in= inB);
   by prop_code rmc_code  arrival_date run_date;
   if (inA) and (inB);
   run;
   ...
   by prop_code tfh_rmc tfh_arvl_stay_dt;
	...
	merge comb_fcst (in= inA rename=(rmc_code = tfh_rmc) )     
      demand_data_summarized 
   ( rename = ( tfh_arvl_stay_dt = arrival_date demand = actual ));
      by prop_code tfh_rmc arrival_date;
   format arrival_date yymmdd10.    actual 8.2 hpf_fcst 8.2 base_fcst 8.2
                                    base_error 8.2 hpf_error 8.2;
   if (inA);
...
*/
    %turn_on_hpf (
                ...
                &by_variables,
                hpfsys.daily_master,
                &run_date_1, /**** need to change to &run_date_1 when files are avail**/
				&run_date_2,
